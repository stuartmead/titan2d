...
void get_rgb_grid(double resolution, double xmin, double xmax, double ymin, double ymax, unsigned char** ingrid,
...
int Get_image(double resolution, double x, double y, unsigned char* r, unsigned char* g, unsigned char* b)
{
    ...
    if(x >= gis_image.ghead.xmin && x <= gis_image.ghead.xmax && y >= gis_image.ghead.ymin && y <= gis_image.ghead.ymax)
    {
...
int Get_raster_id(double resolution, double x, double y, int* category_id)
{
    ...
    if(x >= gis_rast.ghead.xmin && x <= gis_rast.ghead.xmax && y >= gis_rast.ghead.ymin && y <= gis_rast.ghead.ymax)
    {
...
int Get_pile_elevation(double resolution, double x, double y, double& elev, Gis_Grid grid)
{
    if(x >= grid.ghead.xmin && x <= grid.ghead.xmax && y >= grid.ghead.ymin && y <= grid.ghead.ymax)
    {
        x = (x - grid.ghead.xmin) / grid.ghead.resolution;
        y = (grid.ghead.ymax - y) / grid.ghead.resolution;
        if((int) y >= grid.ghead.nrows - 1 || (int) x >= grid.ghead.ncols - 1)
        {
            if((int) y >= grid.ghead.nrows - 1)
                y = grid.ghead.nrows - 1;
            if((int) x >= grid.ghead.ncols - 1)
                x = grid.ghead.ncols - 1;
            elev = grid.elev[(int) y][(int) x];
        }
        else
            elev = interpolate_bilinear_at(grid.ghead.resolution, x, y, grid.elev);
        //		*elev = grid.elev[row][col];
    }
    return 0;
}
    ...
    if(x >= gis_grid.ghead.xmin && x <= gis_grid.ghead.xmax && y >= gis_grid.ghead.ymin && y <= gis_grid.ghead.ymax)
    {
        ...
        else
        {
            for(i = 0; i < 2; i++)
            {
                ...
                {
                    xxcurv[i][j] =
                            ((gis_grid.xslope[row + i - 1][col + j + 1] - gis_grid.xslope[row + i - 1][col + j - 1]) + 2
                                    * (gis_grid.xslope[row + i][col + j + 1] - gis_grid.xslope[row + i][col + j - 1])
                             + (gis_grid.xslope[row + i + 1][col + j + 1] - gis_grid.xslope[row + i + 1][col + j - 1]))
                            / (8 * (float) gis_grid.ghead.resolution);
                    yycurv[i][j] =
                            ((gis_grid.yslope[row + i - 1][col + j - 1] - gis_grid.yslope[row + i + 1][col + j - 1]) + 2
                                    * (gis_grid.yslope[row + i - 1][col + j] - gis_grid.yslope[row + i + 1][col + j])
    ...
    for(row = 1; row < gis_grid.ghead.nrows - 1; row++)
    {
        for(col = 1; col < gis_grid.ghead.ncols - 1; col++)
        {
            ...
            gis_grid.slope[row][col] = (float) sqrt(
                    gis_grid.xslope[row][col] * gis_grid.xslope[row][col] + gis_grid.yslope[row][col]
                            * gis_grid.yslope[row][col]);
        }
    }
    for(col = 1; col < gis_grid.ghead.ncols - 1; col++)
    {
